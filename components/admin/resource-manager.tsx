...
/**
 * The list + create/edit form for one kind of record. Administrators use it against /api/admin; teachers use it against
 * /api/teacher, which only ever returns and accepts their own content (and hides the fields admins decide).
 */
export function ResourceManager({ resourceKey, apiBase = "/api/admin", teacher = false }: { resourceKey: string; apiBase?: string; teacher?: boolean }) {
  const meta = resourceMeta[resourceKey];
  const { push } = useToast();

  const listColumns = meta ? meta.listColumns.filter((c) => !(teacher && meta.fields.find((f) => f.key === c)?.teacherHidden)) : [];
  ...
  const load = async () => {
    const res = await fetch(`${apiBase}/${resourceKey}`);
  ...
  const save = async (data: Record<string, any>) => {
    setSaving(true);
    const url = editing ? `${apiBase}/${resourceKey}/${editing.id}` : `${apiBase}/${resourceKey}`;
    ...
    <div>
      ...
      {showForm && (
        <ResourceFormModal
          ...
          fields={meta.fields.filter((f) => !(teacher && f.teacherHidden))}
          relations={relations}
          initial={editing}
          saving={saving}
